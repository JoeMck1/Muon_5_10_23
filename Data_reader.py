...
folder_path = "C:/Users/jm1n22/test_sftp/GEVP/48I_new"
...
for file_name in file_names:
    ...
    all_data = []
    # 121 x 96 i.e. [(all 96 time-slices for operator 1), (all 96 time-slices for operator 2), ... , (all 96 time-slices for operator 121)]
...
data_array = np.array(all_configs)
# shape (27, 121, 96) = [1000.h5, 1040.h5, ...], 1000.h5 = [[op1 t=1, op1 t=2,...], [op2 t=1, op2 t=2,...]]
no_ops = int(np.sqrt(data_array.shape[1]))
time_slices = data_array.shape[2]

smoothed_data = np.array(
    [
        [reality_fn(data_array[j][i]) for i in range(len(data_array[0]))]
        for j in range(len(data_array))
    ]
)  # removes negligible real or imaginary components

reshaped_data = np.array(
    [
        np.reshape(smoothed_data[i].T, (time_slices, no_ops, no_ops))
        for i in range(len(smoothed_data))
    ]
)  # (27,96,11,11)

# final_data = np.delete(
#    np.delete(reshaped_data, 8, axis=2), 8, axis=3
# )  # remove local vectors
final_data = reshaped_data
# final_data = final_data[:, :, 0:4, 0:4]
