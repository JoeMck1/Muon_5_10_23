...
def bs_mat(configs, no_samples):

    bs = [ (sum([random.choice(configs) for i in range(len(configs))])) /len(configs) for j in range(no_samples) ]

    return np.array(bs)

def fold_matrices(matrices):

    '''Function which symmetrically folds a list of matrices after omitting the first matrix in the list'''

    if len(matrices)%2 == 0:
        folded_matrices = np.array([(matrices[i+1] + matrices[-(i+1)])/2 for i in range(int(len(matrices)/2))])

    elif len(matrices)%2 == 1:
        folded_matrices = np.array([(matrices[i+1] + matrices[-(i+1)])/2 for i in range(int((len(matrices) - 1)/2))])
...
def max_indices(matrix):
    ...
    row_of_max_arg = np.argmax(matrix) // matrix.shape[1] # indices of maximum argument of sparse_mat
...
def reordering_template(ref_mat, unordered_vecs):
    ...
    template_matrix = np.zeros((unordered_vecs.shape[0], unordered_vecs.shape[0]), dtype=float) # zero matrix
    product = abs(np.matmul(ref_mat.T, unordered_vecs)) 

    max_val = np.amax(product)
    while max_val > 0:

        row_of_max_arg = np.argmax(product) // product.shape[1] # indices of maximum argument of sparse_mat
...
def column_swapper(unordered_mat, index, index_destination):
    ...
    unordered_mat[:, [index, index_destination]] = unordered_mat[:, [index_destination, index]]
...
def matrix_reorderer(template, start_val, eigenvalues, eigenvectors):
    ...
    while np.array_equal(template, identity) == False:
        ...
        if start_val == max_index:
            ...
            print("I've cycled through once without being able to produce the matrices of the desired form. Hence two largest column values must be in the same place.")
