...
def GEVP_fn(params, t, t_0):
    ...
    if params[1] >= params[2]:
        penalty = (
            1e6  # Choose a large penalty term to discourage violation of the constraint
        )
        return (
            (1 - params[0]) * np.exp(-params[1] * (t - t_0))
            + params[0] * np.exp(-params[2] * (t - t_0))
            + penalty
        )

    else:
        return (1 - params[0]) * np.exp(-params[1] * (t - t_0)) + params[0] * np.exp(
            -params[2] * (t - t_0)
        )
...
def single_exp_fit(t, data, inv_cov, init_guess, evaluation_number, bounds):
    ...
    result = ls(
        ...
        init_guess,
        args=(t, data, inv_cov),
        max_nfev=evaluation_number,
        bounds=([0, 0], [b, b]),
    )
    return result.x, np.dot(
        np.transpose(LD(result.x, t, data, inv_cov)), LD(result.x, t, data, inv_cov)
    )
...
def double_exp_fit(t, data, inv_cov, init_guess, evaluation_number, bounds):
    ...
    def LD(params, t, data, inv_cov):
        ...
        cholesky_decomp = np.linalg.cholesky(inv_cov)
        return np.dot(
            cholesky_decomp.T, diff_vec
        )  # THIS COULD BE WRONG, MAY NEED TRANSPOSING
...
# def GEVP_fitt(t, t_0, data, inv_cov, init_guess, evaluation_number, bounds=np.inf):
#    b = bounds
#
#    def LD(params, t, data, inv_cov):
...
def GEVP_fit(t, t_0, data, inv_cov, init_guess, evaluation_number):
    def LD(params, t, data, inv_cov):
        diff_vec = GEVP_fn(params, t, t_0) - data
        cholesky_decomp = np.linalg.cholesky(inv_cov)
        return np.dot(cholesky_decomp.T, diff_vec)
    ...
    result = leastsq(
        ...
        init_guess,
        args=(t, data, inv_cov),
        maxfev=evaluation_number,
        ftol=1e-10,
        xtol=1e-10,
    )
    return result[0], np.dot(
        np.transpose(LD(result[0], t, data, inv_cov)), LD(result[0], t, data, inv_cov)
    )
...
# , maxfev=2000, ftol=1e-10, xtol=1e-10
